...
@NamedQuery(name = "Task.findTasksByProject",
        query = "SELECT t FROM TaskEntity t WHERE t.project = :project AND t.erased = false ORDER BY t.priorityId DESC, t.startDate ASC, t.deadline ASC")
...
@NamedQuery(name = "Task.findTasksByProjectOrderedByDate", query = "SELECT t FROM TaskEntity t WHERE t.project.id = :projectId AND t.erased = false ORDER BY t.startDate ASC")
