...
@NamedQuery(name = "ProjectResource.countProjectsFromResource", query = "SELECT COUNT(pr) FROM ProjectResourceEntity pr WHERE pr.resource.id = :resourceId AND pr.quantity > 0")
